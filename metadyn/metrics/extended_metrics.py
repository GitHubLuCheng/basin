...
def strict_escape_rate(
    ...
    ``metadyn.metrics.metrics``, which only checks if n_basins > 1.
...
def per_problem_extended(
    ...
    ----------
    result:
        The :class:`~metadyn.confidence.estimator.AnswerResult` produced by
...
class ExtendedEvalResult:
    ...
    Fields mirror :class:`~metadyn.metrics.metrics.EvalResult` but add
...
def compute_extended_eval(
    ...
    ----------
    results:
        List of :class:`~metadyn.confidence.estimator.AnswerResult` objects.
    problems:
    ...
    """
    from metadyn.metrics.metrics import (
        accuracy as _acc,
